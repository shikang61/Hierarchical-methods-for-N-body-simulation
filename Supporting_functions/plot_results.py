...
def plot_results(x, results, x_label, plot_style, log_x = False, log_y = False, fit_line = False, ax = None, error_label = ""):
    ...
    if type(results) == dict:
        ...
        # plot style
        if plot_style == "line":
            ax.plot(X, Y, label = "Relative error" + error_label)
        elif plot_style == "scatter":
            ax.scatter(X, Y, label = "Relative error" + error_label, s=10)
