...
def draw_box(ax, box, colour, alpha=1, fill=False):
    ax.add_patch(patches.Rectangle((box.coords[0]-box.size/2, box.coords[1]-box.size/2), box.size, box.size, fill=fill, color = colour, alpha=alpha))
...
def plot_tree(quadtree, particles, error=None, animated=False, visualise_interaction=False, visualise_error=False):
    ...
    error: list
        The list of the error for every particles, to be used for visualisation of the error distribution
    animated: bool
    ...
    visualise_error: bool
        If True, the error distribution will be visualised
